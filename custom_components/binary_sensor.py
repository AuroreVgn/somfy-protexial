...
#    logger.info("SOMFY PROTEXIAL - DUMP ELEMENT")
#    logger.info("Type: %s", type(el))
#    logger.info("Value: %r", el)
#    if isinstance(el, dict):
#        logger.info("Keys: %s", list(el.keys()))
#        for k, v in el.items():
#            logger.info("  %s: %r", k, v)
#    else:
...
async def async_setup_entry(
    ...
    _LOGGER.info("SOMFY PROTEXIAL - Nombre d'elements : %d", len(elements))
    for el in elements:
        # log_element(el, _LOGGER)
